...
**Phase 3b (background reservoir regen) ✅ DONE** (commit pending). The first-paint path never blocks on the paid Claude call: `GetCuratedRowsAsync`/`GetCuratedHeroAsync` gained an `allowRegen` flag — when false (non-shuffle `/hero`), a stale/aged/hash-changed reservoir is served as-is with `ReservoirStale=true` instead of regenerating inline. The endpoint then dispatches `CurationService.RegenerateReservoirInBackground` (via the existing `AIUsageDrainHostedService.Run` drain, so it's shutdown-safe + timeout-bounded; `forceRefresh:false` makes it idempotent so a concurrent/fresh reservoir no-ops), deduped by a 2-min `hero-regen:{userId}` IMemoryCache marker, and surfaced as `hero-bg-regen;desc="dispatched"` in Server-Timing. A user-initiated shuffle keeps `allowRegen:true` (may regen synchronously — the user asked). Brand-new user (no reservoir): returns a fallback hero now + regens in the background, so the AI hero appears next load rather than blocking the first. Full solution green (143 Shared + 73 Api + 39 Client), format clean. The regen path itself needs AI-enabled + a stale reservoir to exercise live (code + prod `hero-bg-regen` marker verified; not reproducible in the AI-disabled test harness).

Original Phase 3 spec (retained for reference):
